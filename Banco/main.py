...
@app.route('/transacoes/delete_all', methods=['DELETE'])
def apaga_transacoes():
    if request.method == 'DELETE':
        try:
            num_rows_deleted = db.session.query(Transacao).delete()
            db.session.commit()
            return jsonify({'message': 'Todas as transações foram deletadas.', 'rows_deleted': num_rows_deleted})
        except Exception as e:
            db.session.rollback()
            return jsonify({'message': 'Ocorreu um erro ao deletar as transações.', 'error': str(e)}), 500
    else:
